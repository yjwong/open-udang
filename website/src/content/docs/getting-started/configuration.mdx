...
Tools listed in `allowed_tools` become OpenCode permission rules and are always approved. Command patterns are supported:
...
Run OpenCode inside an isolated environment:
...
OpenShrimp uses OpenCode for model-provider authentication. The simplest path is to start the bot, then send `/connect` in a private chat to open OpenCode's provider connection flow.

You can also provide provider-specific environment variables to the OpenShrimp process, such as `OPENAI_API_KEY`, when your chosen OpenCode provider supports them. See [Environment Variables](/reference/environment/) for deployment guidance.
