...
| `model` | string | No | `null` | Model override in OpenCode provider/model form, such as `openai/gpt-5.5` |
...
Tools listed here become OpenCode permission rules and are always approved without prompting. Command patterns are supported:
...
- **edit, write, apply_patch** — always require manual approval via Telegram inline keyboard, with an option to "Accept all edits" for the session
...
Extra directories the agent can access outside the context directory. OpenShrimp extends path-scoped auto-approval for `read`, `glob`, and `grep` to these directories.
...
When `computer_use: true` is set, the sandbox runs a headless Wayland desktop (labwc compositor, 1280x720) with Chromium and a foot terminal. The agent interacts via MCP tools: `openshrimp_computer_screenshot`, `openshrimp_computer_click`, `openshrimp_computer_type`, `openshrimp_computer_key`, `openshrimp_computer_scroll`, and `openshrimp_computer_toplevel`.
